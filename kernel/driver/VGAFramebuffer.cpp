...
void VGAFramebuffer::write_misc_port(uint8_t byte) {
	outb(0x3C2, byte);
}
...
void VGAFramebuffer::write_index_port(uint16_t port, uint8_t index, uint8_t byte) {
	outb(port, index);
	outb(port+1, byte);
}
...
int VGAFramebuffer::write(void* buffer, size_t size) {
	for (size_t i = 0; i < size; i++) {
		...
		*(char*)(FRAMEBUFFER_ADDR+(addr/4)) = reinterpret_cast<uint8_t*>(buffer)[i];
	}
	m_position += size;
	m_position %= 320*240;
