...
void BochsFramebuffer::init() {
	Paging::the()->map_page(*(RootPageLevel*)get_cr3(), FRAMEBUFFER_CONFIG_ADDR, FRAMEBUFFER_CONFIG_ADDR);
	*(uint16_t*)(FRAMEBUFFER_CONFIG_ADDR+8) = 0;
	...
	for (int i = 0; i < (xres*yres*4); i+=4096) {
		Paging::the()->map_page(*(RootPageLevel*)get_cr3(), FRAMEBUFFER_ADDR+i, FRAMEBUFFER_ADDR+i);
	}
