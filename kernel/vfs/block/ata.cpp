...
int ATABlockNode::read(void* buffer, size_t size) {
	...
	outb(0x1F2, lba_sector_count&0xFF);
	...
	for (size_t i = 0; i < 256*lba_sector_count; i++) {
		uint16_t w = inw(DATA_PORT);
		b[i]=w;
	}

	memcpy((char*)buffer, ((uint8_t*)b) + (m_position%512), size);
	delete[] b;
...
void ATAManager::setup(VFSNode* parent) {
	...
	while (status & 0x80) status = inb(STATUS_PORT);
