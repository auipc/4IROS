...
extern "C" void __ubsan_handle_type_mismatch_v1(TypeMismatchData* mismatch_data, uintptr_t handle) {
	(void)mismatch_data;
	(void)handle;
	//switch (mismatch_data->typecheck_kind) {

	//}
	//printk("kind: %d %s\n", mismatch_data->typecheck_kind, TypeCheckKinds[mismatch_data->typecheck_kind]);
	//printk("UBSAN: type mismatch %s:%d type %d typeinf %d\n", mismatch_data->loc.filename, mismatch_data->loc.line, mismatch_data->type.type_kind, mismatch_data->type.type_info);
	//printk("UBSAN: alignment %x\n", 1<<mismatch_data->log_alignment);
	//printk("Value handle %x\n", handle);
}
