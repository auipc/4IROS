...
FILE _stdin = {.fd = 0, .eof = 0, .buffer = {.buf = stdin_buf, .buf_idx = 0, .buf_sz = 1024}, .buffering_mode = _IOLBF, .error = 0, .pos = 0 };
FILE _stdout = {.fd = 1, .eof = 0, .buffer = {.buf = stdout_buf, .buf_idx = 0, .buf_sz = 1024}, .buffering_mode = _IOLBF,  .error = 0, .pos = 0 };
...
int fgetc(FILE* stream) {
	char ch;
	if(!fread(&ch, 1, 1, stream)) return 0;
	return ch;
}

int fputc(int ch, FILE* stream) {
	return fwrite((char*)&ch, 1, 1, stream);
}
...
size_t fwrite(const void* buffer, size_t size, size_t count, FILE* stream) {
	switch(stream->buffering_mode) {
		...
		case _IOLBF:
			if (!stream->buffer.buf) return 0;
			if (!stream->buffer.buf_sz) return 0;
			if (size*count >= (stream->buffer.buf_sz-stream->buffer.buf_idx)) {
				return 0;
			}
			memcpy(&stream->buffer.buf[stream->buffer.buf_idx], buffer, size*count);
