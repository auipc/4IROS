...
// Compute 16-bit CRC
uint16_t compute_crc_16(uint8_t *buf, size_t sz) {
	uint16_t rem = 0;
	for (int i = 0; i < sz; i++) {
		rem = (rem << 8) ^ crc_16_table[buf[i] ^ ((rem & 0xff00) >> (16 - 8))];
	}
	return rem;
}
...
int rand() {
	...
	uint16_t seed_2 = _seed;
	uint32_t seed_3 = _seed;
	uint64_t seed_4 = _seed;
	asm volatile("rorb $1, %0" : "=m"(seed_1) : "m"(seed_1));
	asm volatile("rorw $1, %0" : "=m"(seed_2) : "m"(seed_2));
	asm volatile("rorl $1, %0" : "=m"(seed_3) : "m"(seed_3));
	asm volatile("rorq $1, %0" : "=m"(seed_4) : "m"(seed_4));
	seed_2 ^= seed_1 >> 2;
	seed_3 ^= compute_crc_16(&seed_2, sizeof(uint16_t));
	seed_4 ^= seed_3 >> 5;
	_seed = seed_4;
