...
// FIXME This only works for nested functions, though I'm convinced you can get
// the tree hierarchy working.
class ProfDefer final {
  public:
	inline ProfDefer() : m_tsc(rdtsc()), m_depth(depth++) {
		if (!s_vec) {
			s_vec = new Vec<Info>();
		}
	}
	inline ProfDefer(int line, const char *func)
		: m_tsc(rdtsc()), m_line(line), m_depth(depth++), m_func(func) {
		if (!s_vec) {
			s_vec = new Vec<Info>();
		}
	}
	inline ~ProfDefer() {
		...
		if (!m_depth) {
			if (m_line && m_func)
				printk("Prof. Defer (⌐■_■): Results are in! Line %d of "
					   "procedure '%s' took roughly %d cycles.",
					   m_line, m_func, tsc_delta);
			else
				printk("Prof. Defer (⌐■_■): Results are in! *Something* took "
					   "roughly %d cycles.",
					   tsc_delta);
			printk("\n");
			for (size_t i = s_vec->size(); i > 0; i--) {
				...
				if (!info.func)
					printk("  Anonymous sub-procedure took roughly %d cycles "
						   "which is %d% of the root procedure",
						   info.func, info.delta, root_percent);
				else
					printk("  Sub-procedure '%s' took roughly %d cycles which "
						   "is %d% of the root procedure",
						   info.func, info.delta, root_percent);
